---
title: "CRAAM: Robust And Approximate Markov decision processes"
...
The library is build around two main data structures: MDP and RMD. **MDP** is the standard model that consists of states $\mathcal{S}$ and actions $\mathcal{A}$. The robust solution for an MDP would satisfy, for example, the following Bellman optimality equation:
...
Note that $p$ is *absolutely continuous* with respect to $P(s,a,\cdot)$. This is a hard requirement for all choices of abiguity (or uncertainty). 
...
- [Doxygen](http://doxygen.org>)  1.8.0+ to generate documentation
...
| Method                  |  Algorithm                           |
| ----------------------- | ------------------------------------
| vi_gs                   | Gauss-Seidel value iteration; runs in a single thread. 
| vi_jac                  | Jacobi value iteration; parallelized with OpenMP.
| mpi_jac                 | Jacobi modified policy iteration; parallelized with OpenMP. Generally, modified policy iteration is vastly more efficient than value iteration.
| vi_jac_fix              | Jacobi value iteration for policy evaluation; parallelized with OpenMP.
...
- `craam.crobust.DiscreteSamples` : Collection of state to state transitions as well as samples of initial states. 
